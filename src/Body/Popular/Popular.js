...
function Welcome() {
  const labels = ['Unilag', 'Petroleum Training Institute', 'Decagon Institute',];
  const icons = [
    <MdMoneyOff />,
    <FiZap />,
    <FaUserCheck />,
    ...
    <section id="popular" className="flex-column main-center cross-center">
      ...
      <div className="flex space-between welcome-items popular-items">
        {labels.map((label, index) => (
          <div key={label}>
            <img className='popular-image' src={image[index]}/>
            <h3 className='school'>{label}</h3>
            <p className='school-detail'>{details[index].slice(0,250)+'......'}</p>
          </div>
        ))}
      </div>
    </section>
